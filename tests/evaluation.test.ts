...
describe("experiment definitions", () => {
  it("ships six valid focused labs", () => {
    ...
    expect(experiments.map((item) => item.id)).toEqual([
  ...
  it("expands record questions with stable ids and labels", () => {
    ...
    expect(expanded[0]).toMatchObject({
      expandedId: "skillsFit__0",
      recordIndex: 0,
      recordLabel: "Alex",
    });
    expect(expanded[1]).toMatchObject({
      expandedId: "skillsFit__1",
      recordIndex: 1,
      recordLabel: "Jordan",
    });
  });
  ...
  it("validates state, definition versions, and expansion limits", () => {
    ...
    expect(() =>
      validateExperimentState(support, { ticket: "x", context: "" }),
    ).toThrow("Account context");
    expect(
      experimentDefinitionSchema.safeParse({ ...support, version: 2 }).success,
    ).toBe(false);
    const github = getExperiment("github-prioritization");
    expect(() =>
      expandQuestions(github, {
        ...github.sampleState,
        issues: Array.from(
          { length: definitionLimits.records + 1 },
          (_, index) => ({ title: String(index), body: "x" }),
        ),
      }),
...
describe("typed answer normalization", () => {
  ...
  it("preserves full Choice, Score, and Noul evidence", () => {
    const choice = normalizeTypedAnswer(
      expanded.find((item) => item.id === "topic")!,
      {
        choice: "billing",
        confidence: 0.9,
        probabilities: { billing: 9, technical: 1, account: 0, general: 0 },
      },
    );
    const score = normalizeTypedAnswer(
      expanded.find((item) => item.id === "urgency")!,
      {
        score: 3.2,
        confidence: 0.7,
    ...
    const noul = normalizeTypedAnswer(
      expanded.find((item) => item.id === "repeatContact")!,
      { noul: 0.82 },
    );
  ...
  it("rejects malformed, incomplete, and mismatched answers", () => {
    ...
    expect(() =>
      normalizeTypedAnswer(choiceQuestion, {
        choice: "billing",
    ...
    expect(() =>
      normalizeTypedAnswer(choiceQuestion, {
        choice: "technical",
        probabilities: { billing: 0.8, technical: 0.2, account: 0, general: 0 },
      }),
    ).toThrow(/highest/);
    expect(() =>
      normalizeTypedAnswer(
        expanded.find((item) => item.id === "urgency")!,
        { score: 8, probabilities: { 0: 0, 1: 0, 2: 0, 3: 0, 4: 1 } },
      ),
    ).toThrow(/outside/);
  });
});
...
describe("deterministic composition", () => {
  it("calculates metrics, ordered rule traces, and rankings", async () => {
    const support = getExperiment("support-orchestration");
    const supportResult = await createMockJevProvider()({
      definition: support,
      state: support.sampleState,
    });
    const policy = evaluatePolicy(support, supportResult.answers);
    expect(
      policy.metrics.every((metric) => Number.isFinite(metric.contribution)),
    ).toBe(true);
    expect(
      policy.rules
        .flatMap((rule) => rule.conditions)
        .every((condition) => typeof condition.passed === "boolean"),
    ).toBe(true);

    const candidates = getExperiment("candidate-matching");
    const candidateResult = await createMockJevProvider()({
      definition: candidates,
      state: candidates.sampleState,
    });
    const ranking = calculateRanking(
      candidates,
      candidates.sampleState,
  ...
  it("computes compatible previous-run deltas", async () => {
    ...
    const firstProvider = await provider({
      definition,
      state: definition.sampleState,
    });
    const first: ExperimentResult = {
      experimentId: definition.id,
      answers: firstProvider.answers,
      telemetry: {
        provider: "Mock",
        model: "mock",
        requestId: null,
      ...
      request: firstProvider.request,
      rawResponse: firstProvider.rawResponse,
    };
