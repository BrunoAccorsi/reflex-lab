...
const validPayload = {
  ...
  answers: {
    action: { type: "choice", choice: "confirm", confidence: 0.91, probabilities: { proceed: 0.03, confirm: 0.91, inspect: 0.04, stop: 0.02 } },
    risk: { type: "score", score: 3.2, confidence: 0.7, probabilities: { 0: 0, 1: 0.05, 2: 0.15, 3: 0.55, 4: 0.25 } },
  ...
  usage: { input_tokens: 40, output_tokens: 12, total_tokens: 52, cost: 0.00012 },
};

describe("OpenRouter Jev Decisions provider", () => {
  const previousKey = process.env.OPENROUTER_API_KEY;
  beforeEach(() => { process.env.OPENROUTER_API_KEY = "test-key"; });
  afterEach(() => { if (previousKey === undefined) delete process.env.OPENROUTER_API_KEY; else process.env.OPENROUTER_API_KEY = previousKey; });

  it("sends native questions and preserves detailed answers and telemetry", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify(validPayload), { status: 200, headers: { "content-type": "application/json", "x-request-id": "req-123" } }));
    const result = await (await createJevProvider(fetchMock))(input);
    expect(result.answers).toHaveLength(6);
    expect(result.answers[0]).toMatchObject({ kind: "choice", selected: "confirm", criteria: definition.questions[0].kind === "choice" ? definition.questions[0].criteria : {} });
    expect(result.answers[1]).toMatchObject({ kind: "score", rawScore: 3.2, normalizedScore: 0.8 });
    expect(result.answers[2]).toMatchObject({ kind: "noul", probabilityYes: 0.88, probabilityNo: 0.12 });
    expect(result).toMatchObject({ requestId: "req-123", usage: { inputTokens: 40, outputTokens: 12, totalTokens: 52 }, cost: 0.00012 });
    const request = JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body));
    expect(request).toMatchObject({ model: "typesafe/jev-1.13", state: { experiment: "Tool-call safety gate" }, questions: { action: { type: "choice" }, risk: { type: "score" }, authorized: { type: "noul" } } });
  ...
  it("builds repeated record questions", () => {
    ...
    const built = buildDecisionRequest({ definition: github, state: github.sampleState }, "test/model");
  ...
  it("rejects missing, unknown, and malformed provider answers", async () => {
    const missing = await createJevProvider(vi.fn().mockResolvedValue(new Response(JSON.stringify({ ...validPayload, answers: {} }), { status: 200 })));
    await expect(missing(input)).rejects.toMatchObject({ code: "malformed_response", message: expect.stringContaining("missing answer") });
    const unknown = await createJevProvider(vi.fn().mockResolvedValue(new Response(JSON.stringify({ ...validPayload, answers: { ...validPayload.answers, surprise: { noul: 1 } } }), { status: 200 })));
    await expect(unknown(input)).rejects.toMatchObject({ code: "malformed_response", message: expect.stringContaining("unknown question") });
  });

  it("surfaces provider, rate-limit, network, and credential errors", async () => {
    const rateLimited = await createJevProvider(vi.fn().mockResolvedValue(new Response(JSON.stringify({ error: { message: "slow down" } }), { status: 429 })));
    await expect(rateLimited(input)).rejects.toMatchObject({ code: "rate_limited", message: expect.stringContaining("slow down") });
    const providerError = await createJevProvider(vi.fn().mockResolvedValue(new Response(JSON.stringify({ error: { message: "bad questions" } }), { status: 400 })));
    await expect(providerError(input)).rejects.toMatchObject({ code: "provider_error", status: 400, message: expect.stringContaining("bad questions") });
    const network = await createJevProvider(vi.fn().mockRejectedValue(new Error("offline")));
