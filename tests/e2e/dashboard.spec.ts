...
test("keeps the lab selector and workspaces usable across screen sizes", async ({ page }) => {
  for (const width of [390, 768, 1024, 1280, 1512, 1600]) {
    ...
    await expect(page.getByRole("button", { name: /Evaluate lab/ })).toBeVisible();
    expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual(width);
    if (width < 1024) {
      const menu = page.getByRole("button", { name: /Capability labs/ });
      const selector = page.getByRole("navigation", { name: "Decision experiments" });
      await expect(menu).toHaveAttribute("aria-expanded", "false");
      await expect(selector).toBeHidden();
      await menu.click();
      await expect(selector).toBeVisible();
      await selector.getByRole("button", { name: /Tool-call safety gate/i }).click();
      await expect(menu).toHaveAttribute("aria-expanded", "false");
