...
export function loadPresets(): StoredPreset[] {
  ...
  try {
    const parsed = JSON.parse(
      window.localStorage.getItem(PRESET_KEY) ?? "[]",
    ) as unknown;
...
export function makePreset(
  ...
  const unique = createId(definition.id);
  return {
    id: unique,
    name,
    definition: { ...definition, id: unique },
...
export function loadPreviousResult(
  ...
  try {
    const parsed = JSON.parse(
      window.sessionStorage.getItem(`${RESULT_PREFIX}${experimentId}`) ??
        "null",
    ) as ExperimentResult | null;
    return parsed?.experimentId === experimentId &&
...
export function savePreviousResult(result: ExperimentResult) {
  window.sessionStorage.setItem(
    `${RESULT_PREFIX}${result.experimentId}`,
