...
import { Activity, ArrowRight, Check, ChevronDown, ChevronRight, Gauge, Layers3, LoaderCircle, Plus, ShieldAlert, Sparkles, Trash2, Wrench } from "lucide-react";
...
export function ReflexLabDashboard() {
  ...
  return <main className="min-h-screen">
    <div className="mx-auto max-w-[1800px] px-4 py-5 sm:px-8 lg:px-10 xl:px-12">
      ...
      <section className="grid items-start gap-7 pb-16 pt-8 lg:grid-cols-[230px_minmax(0,1fr)] xl:gap-9">
        <aside className="min-w-0 lg:sticky lg:top-6"><p className="mb-3 hidden text-xs font-black uppercase tracking-[0.2em] text-ink/45 lg:block">Capability labs</p><button type="button" aria-controls="lab-menu" aria-expanded={labMenuOpen} onClick={() => setLabMenuOpen((open) => !open)} className="flex w-full items-center justify-between gap-4 rounded-2xl border border-ink/10 bg-white px-4 py-3 text-left shadow-card lg:hidden"><span className="min-w-0"><span className="block text-xs font-black uppercase tracking-[0.16em] text-ink/45">Capability labs</span><span className="mt-1 block truncate text-sm font-bold">{definition.title}</span></span><ChevronDown size={19} className={cn("shrink-0 transition-transform", labMenuOpen && "rotate-180")} /></button><nav id="lab-menu" className={cn("mt-2 gap-2 sm:grid-cols-2 lg:mt-0 lg:grid lg:grid-cols-1", labMenuOpen ? "grid" : "hidden")} aria-label="Decision experiments">{catalog.map((item) => <button key={item.id} onClick={() => selectExperiment(item.id)} aria-current={item.id === definition.id ? "page" : undefined} className={cn("group flex min-w-0 w-full items-center justify-between gap-2 rounded-2xl p-3 text-left transition", item.id === definition.id ? "bg-white shadow-card" : "hover:bg-white/60")}><span className="flex min-w-0 items-center gap-3"><span className={cn("grid h-9 w-9 shrink-0 place-items-center rounded-xl text-xs font-black", accentStyles[item.accent])}>{item.eyebrow.slice(0, 2)}</span><span className="min-w-0"><span className="block text-sm font-bold leading-5">{item.title}</span><span className="mt-0.5 block text-xs text-ink/45">{item.eyebrow}</span></span></span><ArrowRight className={cn("shrink-0 opacity-0 transition group-hover:opacity-50", item.id === definition.id && "opacity-100")} size={16} /></button>)}</nav>
          <div className="grain mt-8 hidden rounded-3xl bg-[#e8e8df] p-5 lg:block"><p className="text-xs font-black uppercase tracking-[0.18em] text-ink/50">Typed evidence</p><p className="mt-3 text-sm leading-6 text-ink/75">Every lab exposes calibrated answers, criteria, score legends, and deterministic policy traces.</p></div>
        </aside>
        <div className="min-w-0"><div className="mb-8"><div className="flex flex-wrap items-center gap-x-5 gap-y-3"><p className="text-xs font-black uppercase tracking-[0.2em] text-ink/45">{definition.eyebrow}</p><div className={cn("rounded-full px-4 py-2 text-xs font-black uppercase tracking-[0.12em]", accentStyles[definition.accent])}><Activity className="mr-2 inline" size={14} /> {definition.questions.length} atomic questions</div></div><h1 className="mt-4 text-4xl font-black tracking-[-0.045em] sm:text-5xl lg:text-[3.4rem] lg:leading-[1.08]">{definition.title}</h1><p className="mt-4 max-w-3xl text-base leading-7 text-ink/60">{definition.description}</p></div>
        <div className="grid min-w-0 items-start gap-7 2xl:grid-cols-[minmax(0,1fr)_minmax(0,1.08fr)] 2xl:gap-8">
        <section className="min-w-0">
          ...
          {mutation.error && <Alert className="mt-5 border-coral/30 bg-coral/10 text-[#8f3c2b]"><ShieldAlert className="mr-2 inline" size={17} /><span className="font-bold">{budgetExhausted ? "Budget exhausted." : "Evaluation failed."}</span> {mutation.error.message}{mutation.error.data?.code === "PRECONDITION_FAILED" && <p className="mt-2 text-xs opacity-80">For local work without credentials, set <code>MOCK_JEV=true</code> in <code>.env.local</code>.</p>}</Alert>}
        </section>
        <aside className="min-w-0"><Card className="overflow-hidden"><div className="bg-ink px-5 pt-5 text-white sm:px-7"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-white/55">Decision evidence</p><h2 className="mt-1 text-2xl font-black">Result workspace</h2></div><Gauge size={25} className="text-white/60" /></div><div className="mt-5 flex gap-1 overflow-x-auto" role="tablist" aria-label="Result views">{tabs.map((tab) => <button key={tab} role="tab" aria-selected={activeTab === tab} onClick={() => setActiveTab(tab)} className={cn("whitespace-nowrap border-b-2 px-3 py-3 text-sm font-bold", activeTab === tab ? "border-white text-white" : "border-transparent text-white/55 hover:text-white/85")}>{tab}</button>)}</div></div>
