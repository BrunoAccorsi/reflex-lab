...
function renderInstructions(question: ExpandedQuestion) {
  return question.recordLabel
    ? `${question.instructions} Evaluate record: ${question.recordLabel}.`
...
export function buildDecisionRequest(input: EvaluationInput, model: string) {
    ...
    expanded.map((question) => {
      if (question.kind === "choice")
        return [
          ...
          {
            type: "choice",
            instructions: renderInstructions(question),
  ...
  return {
    ...
    request: {
      model,
      state: {
        experiment: input.definition.title,
...
export function normalizeDecisionsResponse(
  payload: unknown,
  ...
  const unknownIds = Object.keys(response.answers).filter(
    (id) => !expected.has(id),
  );
  if (unknownIds.length > 0)
    throw new Error(
      `Malformed response: unknown question ids: ${unknownIds.join(", ")}`,
    );
  const answers = expanded.map((question) => {
    if (!(question.expandedId in response.answers))
      throw new Error(
        `Malformed response: missing answer for ${question.expandedId}`,
      );
    return normalizeTypedAnswer(
      question,
      response.answers[question.expandedId],
    );
  });
...
function isBudgetExhausted(status: number, message: string | null): boolean {
  ...
  return /(?:insufficient|not enough|out of|exhausted|depleted|no|zero)\s+(?:available\s+|remaining\s+)?(?:credits?|balance|budget|tokens?)(?:\s+remaining)?|(?:credits?|budget|spend(?:ing)?|quota)\s+(?:limit\s+)?(?:exceeded|exhausted|depleted|reached|used up)/i.test(
    message,
  );
}

export async function createJevProvider(
  fetchImpl: typeof fetch = fetch,
): Promise<(input: EvaluationInput) => Promise<JevProviderResult>> {
  ...
  return async (input) => {
    ...
    if (!response.ok) {
      ...
      console.error("[Jev Lab] OpenRouter request failed", {
        status: response.status,
        requestId: response.headers.get("x-request-id"),
        body: raw,
      });
      if (isBudgetExhausted(response.status, providerMessage))
        throw new JevProviderError(
          "You're too late! All free tokens have already been consumed.",
          "budget_exhausted",
          response.status,
        );
      ...
      throw new JevProviderError(
        providerMessage
          ? `OpenRouter returned an error (${response.status}): ${providerMessage}`
          : `OpenRouter returned an error (${response.status}).`,
        "provider_error",
        response.status,
      );
    }
    try {
      return normalizeDecisionsResponse(
        raw,
        expanded,
        model,
        request,
      ...
      throw new JevProviderError(
        error instanceof Error ? error.message : "Malformed decision.",
        "malformed_response",
      );
    }
  };
...
function mockNoulValue(id: string, index: number) {
  ...
  if (lowSignals.some((signal) => id.includes(signal)))
    return 0.12 + index * 0.04;
      ...
      expanded.map((question) => {
          ...
          return [
            question.expandedId,
            {
              ...
              probabilities: Object.fromEntries(
                options.map((option) => [
                  option,
                  option === selected ? 0.86 : remainder,
                ]),
              ),
            },
          ];
        ...
        if (question.kind === "score") {
          ...
          const score = Math.max(
            ...
            max -
              index * 0.7 -
              (question.id.includes("effort") || question.id.includes("risk")
                ? 1.2
                : 0.3),
          );
          ...
          return [
            question.expandedId,
            {
              ...
              confidence: 0.72,
              probabilities: Object.fromEntries(
                question.levels.map((_, levelIndex) => [
                  String(levelIndex),
                  levelIndex === rounded ? 0.72 : 0.28 / Math.max(1, max),
                ]),
              ),
            },
          ];
        ...
        return [
          question.expandedId,
          { type: "noul", noul: mockNoulValue(question.id, index) },
        ];
      }),
    );
    const rawResponse = {
...
export async function evaluateWithJev(input: EvaluationInput) {
  const provider =
    process.env.MOCK_JEV === "true"
      ? createMockJevProvider()
